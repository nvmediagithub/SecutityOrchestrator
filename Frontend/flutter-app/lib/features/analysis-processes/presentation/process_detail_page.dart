...
class _ProcessDetailView extends ConsumerStatefulWidget {
  final AnalysisProcess process;

  const _ProcessDetailView({required this.process});

  @override
  ConsumerState<_ProcessDetailView> createState() => _ProcessDetailViewState();
}

class _ProcessDetailViewState extends ConsumerState<_ProcessDetailView> {
  AnalysisProcess get process => widget.process;
        ...
        children: [
          ...
          if (process.id != null)
            FilledButton.icon(
              ...
              onPressed: () => _confirmDelete(context, process),
        ...
        _AnalysisActions(
          ...
          onCompleteLlm: () async {
            try {
              ...
              messenger.showSnackBar(
                SnackBar(content: Text('Ошибка шага LLM: $error')),
              );
            }
          },
          onSubmitTest: (result) async {
            try {
              ...
              messenger.showSnackBar(
                SnackBar(content: Text('Ошибка отправки теста: $error')),
              );
            }
          },
        ),
...
class _AnalysisActions extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    if (currentStep == null) {
      return const Text('Активных шагов нет.');
    }
  ...
  Future<void> _submit() async {
    ...
    try {
      await widget.onSubmit({
        'status': _success ? 'success' : 'failed',
